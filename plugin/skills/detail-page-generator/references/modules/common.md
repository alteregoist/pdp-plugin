...
각 모듈의 역할, 구성 요소, HTML/TailwindCSS 기본 템플릿을 정의한다.
플랫폼별 파일(`롯데마트몰.md`, `쿠팡.md` 등)에서 이 공통 템플릿을 기반으로 디자인을 오버라이드한다.

모든 모듈은 `<section>` 태그로 감싸며 `data-module-type`과 `data-module-order` 속성을 필수로 포함한다.

> **사용법**: 플랫폼별 파일을 먼저 읽고, 해당 파일에 오버라이드가 없는 모듈은 이 파일의 공통 템플릿을 사용한다.
...
- 배경색: 밝은 그레이(`bg-gray-50`) 또는 브랜드 컬러 연한 톤
...
<section data-module-type="C-1" data-module-order="3" class="py-12 px-6">
  <div class="max-w-[{MAX_WIDTH}px] mx-auto space-y-16">
...
**역할**: 법적 고지, 주의사항, 배송/교환/반품 정보 등 필수 표시 정보. 항상 최하단 배치.
...
4. `max-w-[{MAX_WIDTH}px] mx-auto` 컨테이너 기본 적용 (전체 너비 이미지 제외)
